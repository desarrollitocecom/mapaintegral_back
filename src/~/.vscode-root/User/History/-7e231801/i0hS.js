...
const getIssiInfo = async (id) => {
    try {
        ...
        const punto = await redisClient.geoPos("ubicaciones",id); //hGetAll(`ubicacion:${id}`);
        const position = {latitud: punto[0].latitude, longitud: punto[0].longitude}
        console.log("position IssisController.js:",punto);
